...
model_name = "Intel/neural-chat-7b-v3-3"     # Hugging Face model_id or local model
...
##### 2. QuantizeLLM
Quantizing the model by following the below steps:

```bash
# quantize the model directly use model id in Hugging Face. (recommended)
python scripts/quantize.py --model_name EleutherAI/gpt-j-6b --out_file ne-int4.bin --model_file ne-f32.bin  --one_click_run
