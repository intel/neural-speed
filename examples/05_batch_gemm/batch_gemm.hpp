...
class batch_gemm_t {
  ...
  static_assert(
      (!gemm_t::is_col_major_a) && (!gemm_t::is_col_major_b),
 ...
 public:
  ...
  /// BATCH_GEMM config.
  /// @param args Is the BATCH_GEMM arguments for application-related runtime
  /// variables.
  ...
  static cl::sycl::nd_range<3> get_nd_range(arguments_t& args) {
    ...
    cl::sycl::range<3> group_range =
        get_group_range(args.batch_size, args.matrix_m, args.matrix_n);
  ...
  __XETLA_API KERNEL_FUNC void operator()(
    ...
    uint32_t boundary_n = (start_n + wg_tile_n) > args.matrix_n
        ...
        : (start_n + wg_tile_n);
    uint32_t boundary_m =
        (start_m + wg_tile_m) > (args.matrix_m + batch_id * args.matrix_m)
    ...
    mem_desc_a.init(
        args.matA_base,
        {boundary_k_a, boundary_m, args.matA_ld},
        {start_k_a, start_m});

    // setup for matB
    mem_desc_b.init(
        args.matB_base,
        {boundary_n, boundary_k_b, args.matB_ld},
        {start_n, start_k_b});
