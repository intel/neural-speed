...
template <
    ...
    mem_space mem_space_in_,
    mem_space mem_space_out_,
    uint32_t SIMD_,
    uint32_t thread_num_,
    uint32_t softmax_size_,
    gpu_arch arch_tag>
struct xetla_softmax_fwd_t {
  ...
  __XETLA_API KERNEL_FUNC void operator()(
    ...
    softmax_load_payload.init(
        args->data_in_base,
        SIMD,
        block_height * wg_tile_m,
        SIMD,
        0,
        local_offset_y);
    softmax_store_payload.init(
        args->data_out_base,
        SIMD,
        block_height * wg_tile_m,
        SIMD,
    ...
    for (uint32_t row = 0; row < inner_loop_count; ++row) {
      ...
      softmax_store_payload.template update_tdesc<tdesc_update_dir::y_dir>(
          block_height * thread_num);
    }
  } // void run()
}; // struct xetla_softmax_fwd_t
