...
int gemm_polynomial_result_validate(
  ...
  bool result =
      buff_cmp::xetla_buff_cmp(data, other, "gemm_polynomial validation");
...
void gemm_polynomial_run(int iter) {
  ...
  // [Polynomial] epilogue_t is an elementwise operation that will be applied to
  ...
  // [Polynomial] define arguments for each epilogue_tile_op in
  // chained_tile_op_t<>
  ...
  int64_t ops = 2 * static_cast<int64_t>(matrix_m) * matrix_n * matrix_k +
      6 * static_cast<int64_t>(matrix_m) * matrix_n;
  ...
  ASSERT_EQ(
      0,
      gemm_polynomial_result_validate(
          A,
          B,
          C,
          matrix_m,
