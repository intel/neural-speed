...
int validation(
    data_type* layer_inputs_device,
    data_type* h0_inputs_device,
    std::vector<data_type*> i_weights,
    std::vector<data_type*> h_weights,
    data_type* hidden_outputs_device,
    data_type* layer_outputs_device,
    uint32_t batch_size,
    uint32_t input_size,
  ...
  uint32_t input_weight_size =
      input_size * hidden_size + (layer_size - 1) * hidden_size * hidden_size;
  ...
  auto hidden_outputs = alloc_host_and_copy<data_type>(
      hidden_outputs_device, sequence_length * batch_size * hidden_size, queue);
      ...
      for (uint32_t i = 0; i < batch_size; i++) {
        for (uint32_t j = 0; j < hidden_size; j++) {
          ...
          for (uint32_t kk = 0; kk < input_size; kk++) {
            ...
            reset_gate = float(reset_gate) + float(a_temp) * float(wi_0);
            input_gate = float(input_gate) + float(a_temp) * float(wi_1);
            new_gate = float(new_gate) + float(a_temp) * float(wi_2);
          }
          for (uint32_t kk = 0; kk < hidden_size; kk++) {
            ...
            reset_gate = float(reset_gate) + float(b_temp) * float(wh_0);
            input_gate = float(input_gate) + float(b_temp) * float(wh_1);
            hgate_2 += float(b_temp) * float(wh_2);
          }
          reset_gate = 1 / (1 + std::exp(-float(reset_gate)));
          input_gate = 1 / (1 + std::exp(-float(input_gate)));
          new_gate =
              std::tanh(float(new_gate) + float(reset_gate) * float(hgate_2));
          result = float(1 - float(input_gate)) * float(new_gate) +
              float(input_gate) * float(hidden_input[i * hidden_size + j]);
          one_cell[hidden_size * i + j] = result;
          if (layer_id != layer_size - 1) {
            hidden_out[i * hidden_size + j] = result;
          } else {
            if (std::abs(
                    (float(result) - float(hidden_out[i * hidden_size + j])) /
                    float(hidden_out[i * hidden_size + j])) > 0.03) {
              if (++err_cnt < 100) {
                std::cout << "failed at (" << layer_id << ", " << seq << ", "
                          << i << ", " << j << "), "
                          << " golden: " << float(result) << " != GPU: "
  ...
  if (err_cnt > 0) {
    std::cout
        << "pass rate: "
        << ((float)(layer_size * batch_size * hidden_size * sequence_length - err_cnt) /
            (float)(layer_size * batch_size * hidden_size * sequence_length)) *
            100.0f
        << "% ("
        << (layer_size * batch_size * hidden_size * sequence_length - err_cnt)
...
void gru_run(uint32_t iter) {
  ...
  uint32_t warmup = 10;
  int64_t ops =
      3 * L * (2 * N * F * H + 2 * N * H * H) + (S - 1) * 12 * L * N * H * H;
  ...
  for (uint32_t i = 0; i < iter + warmup; i++) {
    ...
    auto gpu_event = queue.submit([&](handler& cgh) {
      cgh.parallel_for<gru_config>(nd_range, [=](nd_item<3> item) KERNEL_MAIN {
        using xcoder_gru_op = kernel_xcoder_gru_fusion<
            typename gru_config::dtype_in,
            typename gru_config::dtype_acc,
            gru_config::wg_tile_m,
            gru_config::wg_tile_n,
            gru_config::sg_tile_m,
            gru_config::sg_tile_n,
