...
cmpt_configs = {"int8": {"int8", "bf16", "fp32"}, "int4_clip": {"int8", "fp32", "bf16"},  "fp4_e2m1_bnb": {"fp32", "bf16"}, "fp4_e2m1": {"fp32", "bf16"}, "nf4": {"fp32", "bf16"},
                "fp8_e5m2": {"fp32", "bf16"}, "fp8_e4m3": {"fp32", "bf16"}
                }

scale_configs = {"int8": {"fp32", "bf16"}, "int4_clip": {"fp32", "bf16"}, "fp4_e2m1_bnb": {"fp32", "bf16"}, "fp4_e2m1": {"fp32", "bf16"}, "nf4": {"fp32", "bf16"},
                 "fp8_e5m2": {"fp32", "fp8_e8m0"}, "fp8_e4m3": {"fp32", "fp8_e8m0"}}

asym_configs = {"int8", "int4_clip"}
...
@pytest.mark.parametrize("weight_type", ["int8", "int4_clip",  "nf4", "fp4_e2m1_bnb", "fp4_e2m1", "fp8_e5m2", "fp8_e4m3"])
...
def test(m, n, k, blocksize, compute_type, weight_type, scale_type, asym, transpose, add_bias, src_dt, dst_dt, dump_tensor_info=True):
    if compute_type not in cmpt_configs[weight_type] or scale_type not in scale_configs[weight_type]:
        pytest.skip()
    if asym and (weight_type not in asym_configs or (compute_type == "int8" and weight_type == "int8")): #TODO(zhe): add constrain in QBits backend.
        pytest.skip()
    torch.manual_seed(0)
