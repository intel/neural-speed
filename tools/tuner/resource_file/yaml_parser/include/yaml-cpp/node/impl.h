...
inline void Node::EnsureNodeExists() const {
    if (!m_isValid) throw InvalidNode(m_invalidKey);
    if (!m_pNode) {
...
struct as_if {
    ...
    T operator()(const S &fallback) const {
        if (!node.m_pNode) return fallback;

        T t;
        if (convert<T>::decode(node, t)) return t;
        return fallback;
    }
};
...
struct as_if<std::string, S> {
    ...
    std::string operator()(const S &fallback) const {
        ...
        if (node.Type() != NodeType::Scalar) return fallback;
        return node.Scalar();
    }
};
...
struct as_if<T, void> {
    ...
    T operator()() const {
        if (!node.m_pNode) throw TypedBadConversion<T>(node.Mark());
...
inline bool Node::is(const Node &rhs) const {
    if (!m_isValid || !rhs.m_isValid) throw InvalidNode(m_invalidKey);
    if (!m_pNode || !rhs.m_pNode) return false;
...
inline const Node Node::operator[](const Key &key) const {
    ...
    if (!value) { return Node(ZombieNode, key_to_string(key)); }
