...
/// @brief Is the epilogue functor specialized for epilogue_policy_default and
/// Xe architecture.
...
class epilogue_t<
 ...
 private:
  ...
  __XETLA_API static void update_sg_tile_tdesc(
    ...
    int32_t sg_idn =
        (g.get_id() / (wg_size_k * wg_size_q * wg_size_p)) % wg_size_n;
 ...
 public:
  ...
  template <
      typename matAcc_t,
      int _sg_tile_n = sg_tile_n,
      int _sg_tile_p = sg_tile_p>
  __XETLA_API KERNEL_FUNC void operator()(
      [[maybe_unused]] work_group_t& g,
      [[maybe_unused]] matAcc_t matAcc[_sg_tile_n][_sg_tile_p],
      [[maybe_unused]] mem_desc_c_t mem_desc_c,
      [[maybe_unused]] arguments_t args = {},
      [[maybe_unused]] uint32_t slm_base = 0,
      [[maybe_unused]] uint32_t nbarrier_base = 0) {
    using mat_tile_desc = typename matAcc_t::tile_desc;
