...
/// @brief Is the gemm functor for unaligned input, Xe architecture and matrix
/// engine.
template <
...
class gemm_t<
 ...
 private:
  ...
  using cooperative_helper_A_t = subgroup::cooperative_load_helper_t<
      matA_t,
      mem_layout_a,
  ...
  using cooperative_helper_B_t = subgroup::cooperative_load_helper_t<
      matB_t,
      mem_layout_b,
...
#pragma unroll
    ...
    for (uint32_t i = 0; i < args.inner_loop_count; i++) {
      ...
      if (load_idx != 0) {
        matA_local_ld_payload.template update_tdesc<tdesc_update_dir::y_dir>(
            wg_size_y * matA_t::tile_size_y);
        matB_local_ld_payload.template update_tdesc<tdesc_update_dir::y_dir>(
            wg_size_x * matB_t::tile_size_y);
      } else {
        matA_local_ld_payload.template update_tdesc<tdesc_update_dir::y_dir>(
            (1 - num_cyclic) * wg_size_y * matA_t::tile_size_y);
        matB_local_ld_payload.template update_tdesc<tdesc_update_dir::y_dir>(
