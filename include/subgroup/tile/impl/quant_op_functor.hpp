...
struct quant_op_t<
  ...
  template <
      typename mat_out_t,
      typename mat_in_t,
      typename dtype_sat = typename mat_out_t::dtype,
      typename coord_t>
  __XETLA_API KERNEL_FUNC void operator()(
      mat_out_t& mat_out,
      mat_in_t& mat_in,
      ...
      const arguments_t& args) {
    static_assert(
        is_same_layout<mat_out_t, mat_in_t>::value,
        " mat_in and mat_out should be the same layout");
    using matAcc_t = subgroup::
        tile_t<typename mat_in_t::dtype, typename mat_out_t::tile_desc>;
    using mat_sat_t =
