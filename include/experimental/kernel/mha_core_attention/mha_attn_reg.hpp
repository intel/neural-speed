...
struct xetla_mha_attn_reg_fwd_t {
  ...
  using pre_processing_64x384 =
      group::pre_processing_default_t<tile_attr_64x384, arch_tag>;
  using pre_processing_64x512 =
      group::pre_processing_default_t<tile_attr_64x512, arch_tag>;
  using pre_processing_32x1024 =
      group::pre_processing_default_t<tile_attr_32x1024, arch_tag>;
  using pre_processing_16x2048 =
      group::pre_processing_default_t<tile_attr_16x2048, arch_tag>;
  ...
  using gemm_op_64x384_t = group::gemm_t<
      compute_policy_QKT,
      tile_attr_64x384,
      mem_desc_a_QKT,
      mem_desc_b_QKT,
      pre_processing_64x384>;
  using gemm_op_64x512_t = group::gemm_t<
      compute_policy_QKT,
      tile_attr_64x512,
      mem_desc_a_QKT,
      mem_desc_b_QKT,
      pre_processing_64x512>;
  using gemm_op_32x1024_t = group::gemm_t<
      compute_policy_QKT,
      tile_attr_32x1024,
      mem_desc_a_QKT,
      mem_desc_b_QKT,
      pre_processing_32x1024>;
  using gemm_op_16x2048_t = group::gemm_t<
      compute_policy_QKT,
      tile_attr_16x2048,
      mem_desc_a_QKT,
      mem_desc_b_QKT,
  ...
  using mat_128x128_tile_desc_t = subgroup::tile_desc_t<
      ...
      reg_layout::tiled>;
  using mat_128x256_tile_desc_t = subgroup::tile_desc_t<
  ...
  __XETLA_API static void call(sycl::nd_item<3>& item, arguments_t* args) {
    ...
    xetla_nbarrier_t<32, 32, arch_tag> third_nbarr;
    ...
    int valid_block_16x16_x = (tid_x + 1) * 16 * block_16x16_num;
    {
      ...
      if (valid_block_16x16_x <= tru_seqlen)
        ...
        valid_block_16x16_x =
            (tru_seqlen + 15 + 16 * block_16x16_num - valid_block_16x16_x) >> 4;
        bndy_block_num = bndy_block_num +
            (valid_block_16x16_x - block_16x16_num) * 16 - tru_seqlen;
      }
      ...
      if (valid_compute) {
        switch (std_seqlen) {
          case 128: {
            ...
            int start_y_a = all_vert_loop * all_vert_stride + seqlen_entry;
            ...
            matElem_reg_4x16x16.xetla_format<float>().xetla_select<16 * 32, 1>(
                0) = matAcc_128x128.reg * args->Pinv;
          } break;
          case 256: {
          ...
          case 384: {
            ...
            gemm_arg_64x384.matA_base_desc.init(
                {args->matQ_ptr},
                {width_a, height_a, pitch_a},
                {start_x_a, start_y_a});
            ...
            // B transpose
            gemm_arg_64x384.matB_base_desc.init(
                {args->matK_ptr},
                {height_b, width_b, pitch_b},
                {start_y_b, start_x_b});
            ...
            gemm_op_64x384(g_thd32_tid, matAcc_64x384, gemm_arg_64x384);
          ...
          case 512: {
            ...
            gemm_arg_64x512.matA_base_desc.init(
                {args->matQ_ptr},
                {width_a, height_a, pitch_a},
                {start_x_a, start_y_a});
            ...
            // B transpose
            gemm_arg_64x512.matB_base_desc.init(
                {args->matK_ptr},
                {height_b, width_b, pitch_b},
                {start_y_b, start_x_b});
            ...
            gemm_op_64x512(g_thd32_tid, matAcc_64x512, gemm_arg_64x512);
          ...
          case 1024: {
            ...
            gemm_arg_32x1024.matA_base_desc.init(
                {args->matQ_ptr},
                {width_a, height_a, pitch_a},
                {start_x_a, start_y_a});
            ...
            // B transpose
            gemm_arg_32x1024.matB_base_desc.init(
                {args->matK_ptr},
                {height_b, width_b, pitch_b},
                {start_y_b, start_x_b});
            ...
            gemm_op_32x1024(g_thd32_tid, matAcc_32x1024, gemm_arg_32x1024);
          ...
          case 2048: {
            ...
            gemm_arg_16x2048.matA_base_desc.init(
                {args->matQ_ptr},
                {width_a, height_a, pitch_a},
                {start_x_a, start_y_a});
            ...
            // B transpose
            gemm_arg_16x2048.matB_base_desc.init(
                {args->matK_ptr},
                {height_b, width_b, pitch_b},
                {start_y_b, start_x_b});
            ...
            gemm_op_16x2048(g_thd32_tid, matAcc_16x2048, gemm_arg_16x2048);
        ...
        { // softmax
          ...
          xetla_vector<uint32_t, 16> address_fmax =
              xetla_vector_gen<uint32_t, 16>(0, 1);
          int address_offset = (batchid * numhead + headid) * Max_SeqLen +
...
#pragma unroll
              for (int i = 0; i < 16; i++) {
                matElem_reg_4x16x16.xetla_select<16, 1>(16 * i).merge(
                    -1e32, attn_mk_4x16.xetla_select<16, 1>(0) > 0);
              }

              matElem_reg_Max = matElem_reg_4x16x16.xetla_select<16 * 16, 1>(0);
            }

            if (valid_block_16x16_x > 1) {
#pragma unroll
              for (int i = 0; i < 16; i++) {
                matElem_reg_4x16x16.xetla_select<16, 1>(16 * i + 16 * 16 * 1)
                    .merge(-1e32, attn_mk_4x16.xetla_select<16, 1>(16) > 0);
              }
              matElem_reg_Max.merge(
                  matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 1),
                  matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 1) >
                      matElem_reg_Max);
...
#pragma unroll
                for (int i = 0; i < 16; i++) {
                  matElem_reg_4x16x16.xetla_select<16, 1>(16 * i + 16 * 16 * 2)
                      .merge(
                          -1e32, attn_mk_4x16.xetla_select<16, 1>(16 * 2) > 0);
                }
                matElem_reg_Max.merge(
                    matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 2),
                    matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 2) >
                        matElem_reg_Max);
...
#pragma unroll
                  for (int i = 0; i < 16; i++) {
                    matElem_reg_4x16x16
                        .xetla_select<16, 1>(16 * i + 16 * 16 * 3)
                        .merge(
                            -1e32,
                            attn_mk_4x16.xetla_select<16, 1>(16 * 3) > 0);
                  }
                  matElem_reg_Max.merge(
                      matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 3),
                      matElem_reg_4x16x16.xetla_select<16 * 16, 1>(
                          16 * 16 * 3) > matElem_reg_Max);
                }
              }
            }
            ...
            matElem_reg_Max_8.xetla_format<float, 16, 8>()
                ...
                .merge(
                    matElem_reg_Max.xetla_format<float, 16, 16>()
                        .xetla_select<16, 1, 8, 1>(0, 0),
                    matElem_reg_Max.xetla_format<float, 16, 16>()
...
#pragma unroll
            for (int i = 0; i < ((16 * 16) / (2 * 4 * RandSIMD)); i++) {
          ...
          {
            matElem_reg_max_global = xetla_load_global<
                float,
                1,
                data_size::default_size,
                cache_hint::read_invalidate,
...
#pragma unroll
            for (int i = 0; i < 16; i++) {
              ...
              matElem_reg_4x16x16.xetla_select<16 * 16, 1>(0)
                  .xetla_select<16, 1>(i * 16) = xetla_exp<float, 16>(
                  matElem_reg_4x16x16.xetla_select<16 * 16, 1>(0)
                      .xetla_select<16, 1>(i * 16));
            }
...
#pragma unroll
...

          {
            ...
            matElem_reg_Sum = matElem_reg_4x16x16.xetla_select<16 * 16, 1>(0);
            ...
            xetla_tatomic_store_global<
                ...
                atomic_op::fadd,
                arch_tag>(
                (uint64_t)args->Sum_ptr,
                address_fmax,
                matElem_reg_Sum_1.xetla_select<16, 1>(0),
                pred);
          }
...
#pragma unroll
          ...
          {
            matElem_reg_Sum_1 = xetla_load_global<
                float,
                1,
                data_size::default_size,
                cache_hint::read_invalidate,
            ...
            matElem_reg_Sum_1 = xetla_inv<float, 16>(matElem_reg_Sum_1);
...
#pragma unroll
              ...
              if constexpr (Dopt_RandGenflag == true) {
                xetla_vector<uint32_t, 4 * RandSIMD> rand_data;
#pragma unroll
                for (int i = 0; i < ((16 * 16) / (4 * RandSIMD)); i++) {
                  rand_data = Rand_Gen.rand();
                  rand_bit.xetla_select<4 * RandSIMD, 1>(
                      (i * (4 * RandSIMD)) + (16 * 16 * 1)) =
                      rand_data > rand_threshold;
                }
              }
...
#pragma unroll
        ...
        switch (std_seqlen) {
          case 128: {
            ...
            int start_x_c = gemm_op_128x128_t::get_matC_offset_x(g_thd32_tid);
            ...
            if constexpr (Dopt_RandGenflag == false) {
              uint8_t* matMkdpot_byte_ptr = (uint8_t*)(args->matMkdpot_ptr);
              matDpotMk_128x128_payload.init(
                  matMkdpot_byte_ptr,
                  width_c,
                  height_c,
                  pitch_c,
                  start_x_c,
                  start_y_c);
              subgroup::tile_load(matDpotMk_128x128, matDpotMk_128x128_payload);
            }

            xetla_vector<float, 16 * 32> matElem_reg_store =
                matElem_reg_4x16x16.xetla_format<float>()
                    .xetla_select<16 * 32, 1>(0);
            matC_128x128.reg = xetla_cvt<dtype_sfx, float>(matElem_reg_store);

            if constexpr (Dopt_RandGenflag == false) {
              rand_bit.xetla_select<16 * 16 * 2, 1>(0) = matDpotMk_128x128.reg;
            }

            if constexpr (sfx_type_size == 2) {
              xetla_vector<uint16_t, 2 * 16 * 16> drop_mk_w = 0;
              drop_mk_w.xetla_merge(
                  SIGN_BIT_W16, rand_bit.xetla_select<16 * 16 * 2, 1>(0) > 0);
              matC_128x128.reg.xetla_format<uint16_t>() |= drop_mk_w;
            }
            if constexpr (sfx_type_size == 1) {
              xetla_vector<uint8_t, 2 * 16 * 16> drop_mk_b = 0;
              drop_mk_b.xetla_merge(
                  SIGN_BIT_B8, rand_bit.xetla_select<16 * 16 * 2, 1>(0) > 0);
              matC_128x128.reg.xetla_format<uint8_t>() |= drop_mk_b;
            }
          ...
          case 256: {
            ...
            if constexpr (Dopt_RandGenflag == false) {
              uint8_t* matMkdpot_byte_ptr = (uint8_t*)(args->matMkdpot_ptr);
              matDpotMk_128x256_payload.init(
                  matMkdpot_byte_ptr,
                  width_c,
                  height_c,
                  pitch_c,
                  start_x_c,
                  start_y_c);
              subgroup::tile_load(matDpotMk_128x256, matDpotMk_128x256_payload);
            }

            matC_128x256.reg = xetla_cvt<dtype_sfx, float>(matElem_reg_4x16x16);
            ...
            if constexpr (sfx_type_size == 2) {
              ...
              drop_mk_w.xetla_merge(
                  SIGN_BIT_W16, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_128x256.reg.xetla_format<uint16_t>() |= drop_mk_w;
            }
            if constexpr (sfx_type_size == 1) {
              xetla_vector<uint8_t, 4 * 16 * 16> drop_mk_b = 0;
              drop_mk_b.xetla_merge(
                  SIGN_BIT_B8, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_128x256.reg.xetla_format<uint8_t>() |= drop_mk_b;
            }
          ...
          case 384: {
            ...
            int height_c = max_seqlen * (batchid * numhead + headid + 1);
            int pitch_c = max_seqlen;
            int start_x_c = gemm_op_64x384_t::get_matC_offset_x(g_thd32_tid);
            int start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_64x384_t::get_matC_offset_y(g_thd32_tid);

            matC_64x384_payload.init(
                args->matQKT_ptr,
                width_c,
                height_c,
                pitch_c,
            ...
            if constexpr (Dopt_RandGenflag == false) {
              ...
              matDpotMk_64x384_payload.init(
                  ...
                  start_x_c,
                  start_y_c);
              subgroup::tile_load(matDpotMk_64x384, matDpotMk_64x384_payload);
            }

            xetla_vector<float, 3 * 16 * 16> matElem_reg_store =
                matElem_reg_4x16x16.xetla_format<float>()
                    .xetla_select<3 * 16 * 16, 1>(0);
            matC_64x384.reg = xetla_cvt<dtype_sfx, float>(matElem_reg_store);

            if constexpr (Dopt_RandGenflag == false) {
              rand_bit.xetla_select<16 * 16 * 3, 1>(0) = matDpotMk_64x384.reg;
            }

            if constexpr (sfx_type_size == 2) {
              xetla_vector<uint16_t, 3 * 16 * 16> drop_mk_w = 0;
              drop_mk_w.xetla_merge(
                  SIGN_BIT_W16, rand_bit.xetla_select<16 * 16 * 3, 1>(0) > 0);
              matC_64x384.reg.xetla_format<uint16_t>() |= drop_mk_w;
            }
            if constexpr (sfx_type_size == 1) {
              xetla_vector<uint8_t, 3 * 16 * 16> drop_mk_b = 0;
              drop_mk_b.xetla_merge(
                  SIGN_BIT_B8, rand_bit.xetla_select<16 * 16 * 3, 1>(0) > 0);
              matC_64x384.reg.xetla_format<uint8_t>() |= drop_mk_b;
            }
          ...
          case 512: {
            ...
            int height_c = max_seqlen * (batchid * numhead + headid + 1);
            int pitch_c = max_seqlen;
            int start_x_c = gemm_op_64x512_t::get_matC_offset_x(g_thd32_tid);
            int start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_64x512_t::get_matC_offset_y(g_thd32_tid);
            matC_64x512_payload.init(
                args->matQKT_ptr,
                width_c,
                height_c,
                pitch_c,
            ...
            if constexpr (Dopt_RandGenflag == false) {
              ...
              matDpotMk_64x512_payload.init(
                  ...
                  start_x_c,
                  start_y_c);
              subgroup::tile_load(matDpotMk_64x512, matDpotMk_64x512_payload);
            }

            matC_64x512.reg = xetla_cvt<dtype_sfx, float>(matElem_reg_4x16x16);
            ...
            if constexpr (sfx_type_size == 2) {
              ...
              drop_mk_w.xetla_merge(
                  SIGN_BIT_W16, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_64x512.reg.xetla_format<uint16_t>() |= drop_mk_w;
            }
            if constexpr (sfx_type_size == 1) {
              xetla_vector<uint8_t, 4 * 16 * 16> drop_mk_b = 0;
              drop_mk_b.xetla_merge(
                  SIGN_BIT_B8, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_64x512.reg.xetla_format<uint8_t>() |= drop_mk_b;
            }
          ...
          case 1024: {
            ...
            int height_c = max_seqlen * (batchid * numhead + headid + 1);
            int pitch_c = max_seqlen;
            int start_x_c = gemm_op_32x1024_t::get_matC_offset_x(g_thd32_tid);
            int start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_32x1024_t::get_matC_offset_y(g_thd32_tid);

            matC_32x1024_payload.init(
                args->matQKT_ptr,
                width_c,
                height_c,
                pitch_c,
            ...
            if constexpr (Dopt_RandGenflag == false) {
              uint8_t* matMkdpot_byte_ptr = (uint8_t*)(args->matMkdpot_ptr);
              matDpotMk_32x1024_payload.init(
                  matMkdpot_byte_ptr,
                  width_c,
                  height_c,
                  pitch_c,
                  start_x_c,
                  start_y_c);
              subgroup::tile_load(matDpotMk_32x1024, matDpotMk_32x1024_payload);
            }

            matC_32x1024.reg = xetla_cvt<dtype_sfx, float>(matElem_reg_4x16x16);
            ...
            if constexpr (sfx_type_size == 2) {
              ...
              drop_mk_w.xetla_merge(
                  SIGN_BIT_W16, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_32x1024.reg.xetla_format<uint16_t>() |= drop_mk_w;
            }
            if constexpr (sfx_type_size == 1) {
              xetla_vector<uint8_t, 4 * 16 * 16> drop_mk_b = 0;
              drop_mk_b.xetla_merge(
                  SIGN_BIT_B8, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_32x1024.reg.xetla_format<uint8_t>() |= drop_mk_b;
            }

            subgroup::tile_store(matC_32x1024, matC_32x1024_payload);
            xetla_fence<memory_kind::untyped_global>();
          } break;
          case 2048: {
            ...
            int height_c = max_seqlen * (batchid * numhead + headid + 1);
            int pitch_c = max_seqlen;
            int start_x_c = gemm_op_16x2048_t::get_matC_offset_x(g_thd32_tid);
            int start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_16x2048_t::get_matC_offset_y(g_thd32_tid);

            matC_16x2048_payload.init(
                args->matQKT_ptr,
                width_c,
                height_c,
                pitch_c,
            ...
            if constexpr (Dopt_RandGenflag == false) {
              uint8_t* matMkdpot_byte_ptr = (uint8_t*)(args->matMkdpot_ptr);
              matDpotMk_16x2048_payload.init(
                  matMkdpot_byte_ptr,
                  width_c,
                  height_c,
                  pitch_c,
                  start_x_c,
                  start_y_c);
              subgroup::tile_load(matDpotMk_16x2048, matDpotMk_16x2048_payload);
            }

            matC_16x2048.reg = xetla_cvt<dtype_sfx, float>(matElem_reg_4x16x16);
            ...
            if constexpr (sfx_type_size == 2) {
              ...
              drop_mk_w.xetla_merge(
                  SIGN_BIT_W16, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_16x2048.reg.xetla_format<uint16_t>() |= drop_mk_w;
            }
            if constexpr (sfx_type_size == 1) {
              xetla_vector<uint8_t, 4 * 16 * 16> drop_mk_b = 0;
              drop_mk_b.xetla_merge(
                  SIGN_BIT_B8, rand_bit.xetla_select<16 * 16 * 4, 1>(0) > 0);
              matC_16x2048.reg.xetla_format<uint8_t>() |= drop_mk_b;
            }

            subgroup::tile_store(matC_16x2048, matC_16x2048_payload);
            xetla_fence<memory_kind::untyped_global>();
          }
        } // store switch
...
struct xetla_mha_attn_reg_bwd_t {
  ...
  using matC_128x256_tile_desc_t = subgroup::tile_desc_t<
      ...
      matAcc_128x256_t::tile_desc::block_size_y,
      reg_layout::tiled>;
  using matC_64x384_tile_desc_t = subgroup::tile_desc_t<
      matAcc_64x384_t::tile_desc::tile_size_x,
      matAcc_64x384_t::tile_desc::tile_size_y,
      matAcc_64x384_t::tile_desc::block_size_x,
      matAcc_64x384_t::tile_desc::block_size_y,
      reg_layout::tiled>;
  using matC_64x512_tile_desc_t = subgroup::tile_desc_t<
      matAcc_64x512_t::tile_desc::tile_size_x,
      matAcc_64x512_t::tile_desc::tile_size_y,
      matAcc_64x512_t::tile_desc::block_size_x,
      matAcc_64x512_t::tile_desc::block_size_y,
      reg_layout::tiled>;
  using matC_32x1024_tile_desc_t = subgroup::tile_desc_t<
      matAcc_32x1024_t::tile_desc::tile_size_x,
      matAcc_32x1024_t::tile_desc::tile_size_y,
      matAcc_32x1024_t::tile_desc::block_size_x,
      matAcc_32x1024_t::tile_desc::block_size_y,
      reg_layout::tiled>;
  using matC_16x2048_tile_desc_t = subgroup::tile_desc_t<
      matAcc_16x2048_t::tile_desc::tile_size_x,
      matAcc_16x2048_t::tile_desc::tile_size_y,
      matAcc_16x2048_t::tile_desc::block_size_x,
      matAcc_16x2048_t::tile_desc::block_size_y,
      ...
      if (valid_compute) {
        switch (std_seqlen) {
          case 128: {
            ...
            ld_st_start_x_c = gemm_op_128x128_t::get_matC_offset_x(g_thd32_tid);
            ld_st_start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_128x128_t::get_matC_offset_y(g_thd32_tid);

            matW_128x128_payload.init(
                args->matW_ptr,
                ld_st_width_c,
                ld_st_height_c,
                ld_st_pitch_c,
            ...
            matElem_reg_4x16x16.xetla_format<float>().xetla_select<16 * 32, 1>(
                0) = matAcc_128x128.reg;

            if constexpr (sfx_type_size == 1) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 2, 1>(0).merge(
                  1, matW_128x128.reg.xetla_format<int8_t>() < 0);
              matW_128x128.reg.xetla_format<uint8_t>() &= 0x7F;
            }
            if constexpr (sfx_type_size == 2) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 2, 1>(0).merge(
                  1, matW_128x128.reg.xetla_format<int16_t>() < 0);
              matW_128x128.reg.xetla_format<uint16_t>() &= 0x7FFF;
            }

            xetla_vector<float, 2 * 16 * 16> matElem_reg_conv;
            matElem_reg_conv = xetla_cvt<float, dtype_sfx>(matW_128x128.reg);
            matW_reg_4x16x16.xetla_select<16 * 16 * 2, 1>(0) = matElem_reg_conv;
          } break;
          ...
          case 256: {
            ...
            ld_st_start_x_c = gemm_op_128x256_t::get_matC_offset_x(g_thd32_tid);
            ld_st_start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_128x256_t::get_matC_offset_y(g_thd32_tid);

            matW_128x256_payload.init(
                args->matW_ptr,
                ld_st_width_c,
                ld_st_height_c,
                ld_st_pitch_c,
            ...
            matElem_reg_4x16x16.xetla_format<float>()
                .xetla_select<16 * 16 * 4, 1>(0) = matAcc_128x256.reg;

            if constexpr (sfx_type_size == 1) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_128x256.reg.xetla_format<int8_t>() < 0);
              matW_128x256.reg.xetla_format<uint8_t>() &= 0x7F;
            }
            if constexpr (sfx_type_size == 2) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_128x256.reg.xetla_format<int16_t>() < 0);
          ...
          case 384: {
            ...
            ld_st_start_x_c = gemm_op_64x384_t::get_matC_offset_x(g_thd32_tid);
            ld_st_start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_64x384_t::get_matC_offset_y(g_thd32_tid);
            matW_64x384_payload.init(
                args->matW_ptr,
                ld_st_width_c,
                ld_st_height_c,
                ld_st_pitch_c,
            ...
            matElem_reg_4x16x16.xetla_format<float>()
                .xetla_select<16 * 16 * 3, 1>(0) = matAcc_64x384.reg;

            if constexpr (sfx_type_size == 1) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 3, 1>(0).merge(
                  1, matW_64x384.reg.xetla_format<int8_t>() < 0);
              matW_64x384.reg.xetla_format<uint8_t>() &= 0x7F;
            }
            if constexpr (sfx_type_size == 2) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 3, 1>(0).merge(
                  1, matW_64x384.reg.xetla_format<int16_t>() < 0);
              matW_64x384.reg.xetla_format<uint16_t>() &= 0x7FFF;
            }

            xetla_vector<float, 3 * 16 * 16> matElem_reg_conv;
            matElem_reg_conv = xetla_cvt<float, dtype_sfx>(matW_64x384.reg);
            matW_reg_4x16x16.xetla_select<16 * 16 * 3, 1>(0) = matElem_reg_conv;
          } break;
          ...
          case 512: {
            ...
            ld_st_start_x_c = gemm_op_64x512_t::get_matC_offset_x(g_thd32_tid);
            ld_st_start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_64x512_t::get_matC_offset_y(g_thd32_tid);
            matW_64x512_payload.init(
                args->matW_ptr,
                ld_st_width_c,
                ld_st_height_c,
                ld_st_pitch_c,
            ...
            matElem_reg_4x16x16.xetla_format<float>()
                .xetla_select<16 * 16 * 4, 1>(0) = matAcc_64x512.reg;

            if constexpr (sfx_type_size == 1) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_64x512.reg.xetla_format<int8_t>() < 0);
              matW_64x512.reg.xetla_format<uint8_t>() &= 0x7F;
            }
            if constexpr (sfx_type_size == 2) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_64x512.reg.xetla_format<int16_t>() < 0);
          ...
          case 1024: {
            ...
            ld_st_start_x_c = gemm_op_32x1024_t::get_matC_offset_x(g_thd32_tid);
            ld_st_start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_32x1024_t::get_matC_offset_y(g_thd32_tid);
            matW_32x1024_payload.init(
                args->matW_ptr,
                ld_st_width_c,
                ld_st_height_c,
                ld_st_pitch_c,
            ...
            matElem_reg_4x16x16.xetla_format<float>()
                .xetla_select<16 * 16 * 4, 1>(0) = matAcc_32x1024.reg;

            if constexpr (sfx_type_size == 1) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_32x1024.reg.xetla_format<int8_t>() < 0);
              matW_32x1024.reg.xetla_format<uint8_t>() &= 0x7F;
            }
            if constexpr (sfx_type_size == 2) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_32x1024.reg.xetla_format<int16_t>() < 0);
          ...
          case 2048: {
            ...
            ld_st_start_x_c = gemm_op_16x2048_t::get_matC_offset_x(g_thd32_tid);
            ld_st_start_y_c = (batchid * numhead + headid) * max_seqlen +
                all_vert_loop * all_vert_stride +
                gemm_op_16x2048_t::get_matC_offset_y(g_thd32_tid);
            matW_16x2048_payload.init(
                args->matW_ptr,
                ld_st_width_c,
                ld_st_height_c,
                ld_st_pitch_c,
            ...
            matElem_reg_4x16x16.xetla_format<float>()
                .xetla_select<16 * 16 * 4, 1>(0) = matAcc_16x2048.reg;

            if constexpr (sfx_type_size == 1) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_16x2048.reg.xetla_format<int8_t>() < 0);
              matW_16x2048.reg.xetla_format<uint8_t>() &= 0x7F;
            }
            if constexpr (sfx_type_size == 2) {
              Sign_reg_4x16x16.xetla_select<16 * 16 * 4, 1>(0).merge(
                  1, matW_16x2048.reg.xetla_format<int16_t>() < 0);
        ...
        // softmax
        {
          ...
          matElem_reg_4x16x16.xetla_format<float>()
              .xetla_select<16 * 16 * 2, 1>(0)
              .merge(0.0, Sign_reg_4x16x16.xetla_select<16 * 16 * 2, 1>(0) > 0);
          ...
          if (valid_block_16x16_x > 2) {
            matElem_reg_4x16x16.xetla_format<float>().xetla_select<16 * 16, 1>(
                16 * 16 * 2) *=
                matW_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 2);

            matElem_reg_4x16x16.xetla_format<float>()
                .xetla_select<16 * 16, 1>(16 * 16 * 2)
                .merge(
                    0.0,
                    Sign_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 2) > 0);
            ...
            if (valid_block_16x16_x > 3) {
              matElem_reg_4x16x16.xetla_format<float>()
                  .xetla_select<16 * 16, 1>(16 * 16 * 3) *=
                  matW_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 3);

              matElem_reg_4x16x16.xetla_format<float>()
                  .xetla_select<16 * 16, 1>(16 * 16 * 3)
                  .merge(
                      0.0,
                      Sign_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 3) >
                          0);
...
#pragma unroll
          for (int i = 0; i < 16; i++) {
            ...
            matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 0)
                .xetla_select<16, 1>(i * 16) =
                matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 0)
                    .xetla_select<16, 1>(i * 16) -
                matW_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 0)
            ...
            matElem_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 0)
                .xetla_select<16, 1>(i * 16) *= args->Pinv;
          }

          if (valid_block_16x16_x > 1) {
#pragma unroll
            for (int i = 0; i < 16; i++) {
              matW_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 1)
                  .xetla_select<16, 1>(i * 16) =
                  matW_reg_4x16x16.xetla_select<16 * 16, 1>(16 * 16 * 1)
                      .xetla_select<16, 1>(i * 16) *
                  matElem_reg_Sum_1[i];
