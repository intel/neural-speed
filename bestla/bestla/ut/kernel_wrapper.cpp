...
class UT_LayerNormalization {
 public:
  UT_LayerNormalization() {
    UT_START();
    ut<float, BTLA_ISA::AVX512F>(4096, false, true);  
    ut<float, BTLA_ISA::AVX512F>(111, false, true);  
    ut<float, BTLA_ISA::AVX512F>(111, true, true);   
    ut<float, BTLA_ISA::AVX2>(4096, false, true);  
    ut<float, BTLA_ISA::AVX2>(111, false, true);  
    ut<float, BTLA_ISA::AVX2>(111, true, true);   
  }
  template <typename T, BTLA_ISA ISA>
  void ut(int norm_size, bool simplified, bool hasbias) {
    ...
    kernel::wrapper::LayerNormalization::forward<BTLA_ISA::NoSIMD>(src.data(), scale.data(),
                                                                   hasbias ? bias.data() : nullptr, 0.00001f, norm_size,
                                                                   ref.data(), &mean, &mean_square, simplified);
    kernel::wrapper::LayerNormalization::forward<ISA>(src.data(), scale.data(), hasbias ? bias.data() : nullptr,
                                                      0.00001f, norm_size, dst.data(), &mean, &mean_square, simplified);
