...
//     quanW.resize(row * col);
//     quanW.fill_rand(-8, 7);
//     for (int i = 0; i < row * col; i++) quanW.data()[i] = (quanW.data()[i] * 16) & 0xe0;
//     std::vector<bit2x4> bit2_data(row * col / 4);
//     std::vector<bit1x8> bit1_data(row * col / 8);
//     std::vector<int8_t> ref(row * col);
//     std::vector<int8_t> tar(row * col);
...
//         bit2_interleave(const_cast<int8_t*>(quanW.data() + i * col + j), interleave_buf);
//         for (int k = 0; k < 32; k++) {
//           bit2_data[i * col / 4 + j / 4 + k].a = interleave_buf[4 * k] >> 5;
//           bit2_data[i * col / 4 + j / 4 + k].b = interleave_buf[4 * k + 1] >> 5;
//           bit2_data[i * col / 4 + j / 4 + k].c = interleave_buf[4 * k + 2] >> 5;
//           bit2_data[i * col / 4 + j / 4 + k].d = interleave_buf[4 * k + 3] >> 5;
//         }
//       }
//     }
...
//         bit1_data[i * col / 8 + j / 8].a = quanW.data()[i * col + j] >> 7;
//         bit1_data[i * col / 8 + j / 8].b = quanW.data()[i * col + j + 1] >> 7;
//         bit1_data[i * col / 8 + j / 8].c = quanW.data()[i * col + j + 2] >> 7;
//         bit1_data[i * col / 8 + j / 8].d = quanW.data()[i * col + j + 3] >> 7;
//         bit1_data[i * col / 8 + j / 8].e = quanW.data()[i * col + j + 4] >> 7;
//         bit1_data[i * col / 8 + j / 8].f = quanW.data()[i * col + j + 5] >> 7;
//         bit1_data[i * col / 8 + j / 8].g = quanW.data()[i * col + j + 6] >> 7;
//         bit1_data[i * col / 8 + j / 8].h = quanW.data()[i * col + j + 7] >> 7;
//       }
//     }

//     kernel::avx512f::decompress_kblock_s3_s8fp<BTLA_DTYPE::S3_CLIP, int8_t>(bit2_data.data(), bit1_data.data(),
//                                                                             ref.data(), 0, row * col, nullptr, -1);
//     kernel::jit::DecompresssS3::forward_avx512f(bit2_data.data(), bit1_data.data(), tar.data(), row * col);
//     buffer_error(tar.data(), ref.data(), ref.size());
//   }
// };
// static UT_DecompressS3S8 sUT_DecompressS3S8;
