                    ...
                    if [[ "${model}" == "whisper" ]];then NEURAL_SPEED_VERBOSE=1 OMP_NUM_THREADS=$cores_per_instance numactl -m 0 -C 0-$(($cores_per_instance - 1)) \
                        ...
                        if [[ ${input} == "1024" && ${cores_per_instance} == "32" ]]; then
                            ...
                            if [[ "${model}" == "llama"* || "${model}" == "gptj-6b" ]]; then
                                python ./scripts/cal_acc.py --model_name ${model_path} --init_from_bin ${model}-${precision}.bin --batch_size 8 --tasks piqa 2>&1 | tee  ${WORKSPACE}/accuracy-${logs_file}
                            else
                                python ./scripts/cal_acc.py --model_name ${model_path} --init_from_bin ${model}-${precision}.bin --tasks lambada_openai,boolq,piqa,hellaswag --batch_size 1  2>&1 | tee  ${WORKSPACE}/accuracy-${logs_file}
                            fi
                        else
                            echo "-------- Accuracy End --------"
                        fi
