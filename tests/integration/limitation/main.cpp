...
TEST(slm, check_aligment) {
  EXPECT_TRUE(
      (core::general_1d<gpu_arch::XeHpc, uint32_t>::template check_restriction<
          1>(0)));
  EXPECT_FALSE(
      (core::general_1d<gpu_arch::XeHpc, float>::template check_restriction<1>(
          1)));
  EXPECT_FALSE(
      (core::general_1d<gpu_arch::XeHpc, float>::template check_restriction<1>(
          2)));
...
TEST(general_1d, check_alignment) {
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, fp16>::check_alignment(
      (fp16*)0x40000, 0)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, fp16>::check_alignment(
      (fp16*)0x40000, 1)));
  EXPECT_TRUE((kernel::general_1d<gpu_arch::XeHpc, fp16>::check_alignment(
      (fp16*)0x40000, 2)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, fp16>::check_alignment(
      (fp16*)0x40000, 3)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, uint8_t>::check_alignment(
      (uint8_t*)0x40000, 6)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, uint8_t>::check_alignment(
      (uint8_t*)0x40000, 5)));
  EXPECT_TRUE((kernel::general_1d<gpu_arch::XeHpc, float>::check_alignment(
      (float*)0x40000, 1)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, float>::check_alignment(
      (float*)0x40001, 1)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, float>::check_alignment(
      (float*)0x40002, 1)));
  EXPECT_FALSE((kernel::general_1d<gpu_arch::XeHpc, float>::check_alignment(
...
TEST(block_2d, check_load) {
  ...
  EXPECT_FALSE(
      (core::block_2d<gpu_arch::XeHpc, fp16>::template check_load<false, false>(
          td)));
...
TEST(block_2d, check_store) {
  ...
  xetla_fill_tdesc<fp16, 100, 200, 1>(
      td.xetla_format<uint32_t>(), (fp16*)4096, 32, 64, 32, 0, 0);
  EXPECT_FALSE((core::block_2d<gpu_arch::XeHpc, fp16>::check_store(td)));
  ...
  xetla_fill_tdesc<fp16, 32, 8, 1>(
      td.xetla_format<uint32_t>(), (fp16*)4096, 32, 64, 32, 0, 0);
  EXPECT_TRUE((core::block_2d<gpu_arch::XeHpc, fp16>::check_store(td)));
}

TEST(block_2d, check_tensor) {
  using check_2d = kernel::block_2d<gpu_arch::XeHpc, float>;
