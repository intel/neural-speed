...
void ln_fwd_run() {
  ...
  auto buffer_x = alloc_device_and_init<data_type_x>(
      ...
      [](data_type_x* data, size_t idx) {
        data[idx] = static_cast<data_type_x>((random_float() - 0.5) * 10);
      },
      queue,
      device,
      context);
  ...
  auto buffer_rand_offset = alloc_device_and_init<uint64_t>(
      1,
      [](uint64_t* data, size_t idx) { data[idx] = static_cast<uint64_t>(0); },
      queue,
      device,
      context);
  ...
  try {
    auto e_esimd = queue.submit([&](handler& cgh) {
      cgh.parallel_for<test>(nd_range, [=](nd_item<3> item) KERNEL_MAIN {
        ...
        using ln_fwd_func = ln_fwd_func_t<
            data_type_x,
            data_type_y,
            data_type_weight,
            data_type_acc,
  ...
  auto buffer_gamma_host =
      alloc_host_and_copy<data_type_weight>(buffer_gamma, size_gamma, queue);
  auto buffer_beta_host =
      alloc_host_and_copy<data_type_weight>(buffer_beta, size_beta, queue);
  auto buffer_mu_host =
  ...
  auto buffer_resAdd_host =
      alloc_host_and_copy<data_type_x>(buffer_resAdd, size_resAdd, queue);
  auto buffer_mask_host =
      alloc_host_and_copy<uint8_t>(buffer_mask, size_mask, queue);
  auto buffer_bias_dropout_res_host =
      alloc_host_and_copy<data_type_x>(buffer_bias_dropout_res, size_in, queue);

  // validation
  ASSERT_EQ(
      0,
      (ln_fwd_result_validate<
          data_type_x,
          data_type_weight,
          data_type_y,
          data_type_acc>(
