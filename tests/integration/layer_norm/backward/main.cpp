...
void ln_bwd_run() {
  ...
  auto dy_in = alloc_device_and_init<data_type_y>(
      ...
      [](data_type_y* data, size_t idx) {
        data[idx] = static_cast<data_type_y>((random_float() - 0.5) * 10);
      },
      queue,
      device,
      context);
  ...
  cl::sycl::range<3> final_local_range{
      1,
  ...
  try {
    auto e_esimd_bwd0 = queue.submit([&](handler& cgh) {
      cgh.parallel_for<test>(nd_range, [=](nd_item<3> item) KERNEL_MAIN {
        using ln_bwd_func = ln_bwd_func_t<
            data_type_y,
            data_type_x,
            data_type_weight,
            data_type_acc,
    ...
    auto e_esimd_bwd1 = queue.submit([&](handler& cgh) {
      cgh.parallel_for(final_range, [=](nd_item<3> item) KERNEL_MAIN {
        ...
        constexpr uint32_t barrier_count = ln_bwd_final_func::barrier_count;
        if constexpr (slm_size != 0) {
          xetla_local_init<slm_size>();
        }
        if constexpr (barrier_count != 0) {
  ...
  auto dy_in_host = alloc_host_and_copy<data_type_y>(dy_in, size_dy_in, queue);
  auto grad_in_host =
  ...
  auto gamma_in_host =
      alloc_host_and_copy<data_type_weight>(gamma_in, size_gamma_in, queue);
  auto buffer_mu_host =
  ...
  auto dbias_host = alloc_host_and_copy<data_type_x>(dbias, size_dbias, queue);
