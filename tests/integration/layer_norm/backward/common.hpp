...
int ln_bwd_result_validate(
    data_type_y* dy_in,
    data_type_y* grad_in,
    data_type_x* x_in,
    data_type_weight* gamma_in,
    data_type_acc* mu_in,
    data_type_acc* rs_in,
    data_type_x* dx_out,
    data_type_weight* dgamma_out,
    data_type_weight* dbeta_out,
    int m,
    int n,
    [[maybe_unused]] int mask_n,
    [[maybe_unused]] int sg_size_n,
    data_type_x* dx_resAdd_out,
    data_type_x* dbias_out,
    uint8_t* buffer_mask,
    float drop_out_scale_inv,
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      data_type_acc y_temp = dy_acc[i * n + j] * (data_type_acc)gamma_in[j];
  ...
  result &= buff_cmp::xetla_buff_cmp(gpu_dgamma, cpu_dgamma, "compare dgamma");
  ...
  if (is_bias_dropout_resAdd_fuse) {
    ...
    result &= buff_cmp::xetla_buff_cmp(gpu_dbias, cpu_dbias, "compare dbias");
  }
...
class ln_bwd_1_bf16 {
 public:
  ...
  using data_type_weight = bf16;
  using data_type_acc = float;
  static constexpr ln_bwd_fused_kind ln_fused_op_kind = ln_bwd_fused_kind::none;
};
