...
static void data_transformer_run() {
  ...
  auto buffer_in = alloc_device_and_init<data_type_in>(
      ...
      [](data_type_in* data, size_t idx) {
        data[idx] = static_cast<data_type_in>((random_float() - 0.5) * 10);
      },
      queue,
      device,
      context);
  ...
  try {
    auto e_esimd = queue.submit([&](handler& cgh) {
      ...
      cgh.parallel_for<Test>(nd_range, [=](nd_item<3> item) KERNEL_MAIN {
        using data_transformer_attr = gpu::xetla::kernel::
            data_transformer_attr_t<wg_tile_n, wg_tile_m, sg_tile_n, sg_tile_m>;
        using data_transformer = gpu::xetla::kernel::xetla_data_transformer<
            data_type_in,
            data_type_out,
            data_type_acc,
        ...
        constexpr uint32_t slm_size = data_transformer::get_slm_size::size;
        if constexpr (slm_size != 0) {
          xetla_local_init<slm_size>();
        }

        data_transformer_func<
            data_type_in,
            data_type_out,
            data_type_acc,
            wg_tile_n,
