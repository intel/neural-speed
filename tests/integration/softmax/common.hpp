...
template <
    ...
    typename data_type_coff_in,
    typename data_type_out,
    typename data_type_acc>
int bwd_reduction_result_validate(
    data_type_in* in_ptr,
    data_type_coff_in* coff_in_ptr,
    data_type_out* out_ptr,
    int m,
    int n,
...
int fwd_reduction_result_validate(
    data_type_in* in_ptr,
    data_type_out* out_ptr,
    int m,
    int n,
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      softmax_acc[i * n + j] = std::exp(
          (data_type_acc(in_ptr[i * n + j]) - softmax_max[i]) * sqrt_dk_inv);
    }
  }
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
