...
void fpu_fp32_gemm_run(int iter) {
  ...
  using compute_attr =
      xetla::group::compute_attr_t<data_type_acc, data_type_acc, data_type_acc>;
  using perf_tuning_knob = xetla::group::
      perf_tuning_knob_t<sg_tile_k, prefetch_distance, periodic_sync_interval>;

  using compute_policy = xetla::group::compute_policy_default_fpu<
      compute_attr,
      perf_tuning_knob,
      gpu_arch::XeLpg>;
  ...
  using epilogue_t = xetla::group::epilogue_t<
      xetla::group::epilogue_policy_default<gpu_arch::XeLpg>,
      tile_shape,
      mem_desc_c_t>;

  using group_swizzle = xetla::kernel::group_swizzle_default<gpu_arch::XeLpg>;
  ...
  typename gemm_op_t::arguments_t gemm_arg(
      matrix_m, matrix_k, matrix_n, A_d, lda, B_d, ldb, C_d, ldc, Acc_d, Cnt_d);
  ...
  ASSERT_EQ(
      ...
      gemm_result_validate(
          A_h, B_h, C_h, matrix_m, matrix_k, matrix_n, layout_a, layout_b));
