    ...
    for (int i = 0; i < iter; i++) {
      ...
      auto e_esimd = queue.submit([&](handler& cgh) {
        cgh.parallel_for<Test>(nd_range, [=](nd_item<3> item) KERNEL_MAIN {
          using igemm_quantize_functor = igemm_quantize_func<
              data_type_a,
              data_type_b,
              data_type_c,
              data_type_param,
          ...
          constexpr uint32_t slm_size = igemm_quantize_functor::slm_size;
          ...
          igemm_quantize_functor::run(
              item, A, B, C, scale, offset, matrix_m, matrix_n, matrix_k);
        });
      });
      e_esimd.wait();
      prof.cpu_end();
