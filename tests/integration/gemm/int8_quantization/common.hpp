...
int gemm_result_validate(
    ...
    data_type_param* scale_device,
    data_type_param* offset_device,
    int m,
    int k,
    int n,
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      ...
      for (int kk = 0; kk < k; kk++) {
        data_type_a a_temp = is_col_major_a ? A[i + kk * m] : A[i * k + kk];
        data_type_b b_temp = is_col_major_b ? B[kk + j * k] : B[kk * n + j];
        result = result + data_type_acc(a_temp) * data_type_acc(b_temp);
      }
      // Quantization with saturation
      float result_fp32 = result;
      float result_scaled = std::round(result_fp32 * scale[j] + offset[j]);
