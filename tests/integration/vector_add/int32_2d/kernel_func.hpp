...
KERNEL_FUNC inline void load2D_D32_A64_CA_CA(
    xetla_vector_ref<T1, n1> __REF__ data,
    xetla_vector<T2, n2> payload) {
  // uses 2D block read
  ...
  desc += numDst << 20; // Response length. 31 represents 32 with hidden -1. The
                        // rest doesn't use -1.
...
KERNEL_FUNC inline void prefetch2D_D32_A64_CA_CA(xetla_vector<T1, n1> payload) {
  ...
  desc += 0x0 << 15; // transpose off
  desc += 0x4 << 17; // L1 cached L2 cached
  desc += 0 << 20; // Response length, prefetch=0. 31 represents 32 with hidden
                   // -1. The rest doesn't use -1.
  desc += 0x1 << 25; // Msg length
  desc += 0x0 << 29; // flat address
...
KERNEL_FUNC inline void store2D_D32_A64_WB_WB(
    xetla_vector<T1, n1> payload,
    xetla_vector<T2, n2> result) {
  // uses 2D block write
  const uint32_t exDesc = 0;
  ...
  desc += 0 << 20; // Response length. 31 represents 32 with hidden -1. The rest
                   // doesn't use -1.
  desc += 0x1 << 25; // Msg length
  desc += 0x0 << 29; // flat address
...
KERNEL_FUNC inline void vector_add_func(
  ...
  A_load_msg.xetla_select<2, 1>(TWOD_BUF_X_COORD_IN_ELEM) =
      xy_A; // block x start. points to the right most column for more
            // efficiency prefetch of two cache lines per line
  ...
  for (int iy = 0; iy < BLOCK_SIZE; iy += SIMD) {
    ...
    for (int ix = 0; ix < BLOCK_SIZE; ix += SIMD) {
      ...
      load2D_D32_A64_CA_CA<dtype, SIMD * SIMD, uint32_t, 16>(
          A_buffer.xetla_format<dtype>(), A_load_msg.xetla_format<uint32_t>());

      load2D_D32_A64_CA_CA<dtype, SIMD * SIMD, uint32_t, 16>(
          B_buffer.xetla_format<dtype>(), B_load_msg.xetla_format<uint32_t>());

      for (int irow = 0; irow < SIMD; irow++) {
        C_buffer_2d.row(irow) = A_buffer_2d.row(irow) + B_buffer_2d.row(irow);
      }
