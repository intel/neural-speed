...
struct buff_vals {
  ...
  /// @brief Initializes buff_vals<dtype> structure with a one-to-one mapping to
  /// an input buffer.
  /// @tparam dtype Datatype of the output structure and input buffer.
  ...
  /// @brief Converts a buffer array into a buff_vals<dtype> structure for
  /// convenient representation in xetla_buff_cmp<Tx, Ty>.
  /// @tparam dtype Datatype of the output structure and input buffer.
  /// @param data Pointer to buffer of input data.
  /// @param Blocky By default used to define size of input buffer. If input is
  /// organized as tensor (meaning, by blocks) then defines block height.
  /// @param Blockx Default value is 1 for non-tensor buffers, otherwise
  /// describes tensor block width.
  /// @param Sizex Default value is 1 for non-tensor buffers, otherwise
  /// describes tensor pitch.
...
/// @param Blocky By default used to define size of input buffer. If input is
/// organized as tensor (meaning, by blocks) then defines block height.
/// @param Blockx Default value is 1 for non-tensor buffers, otherwise describes
/// tensor block width.
/// @param Sizex Default value is 1 for non-tensor buffers, otherwise describes
/// tensor pitch.
/// @return buff_vals<dtype> Is the output structure describing the input
/// buffer.
///
...
rel_abs_vals xetla_get_rte_and_ate(T1& v1, T2& v2) {
  ...
  auto get_rte = [=](dtype1 a, dtype2 b) {
    return get_ate(a, b) / fmax(1E-10, fmax(fabs((double)a), fabs((double)b)));
  };
...
bool _handle_fp_types(
  ...
  for (size_t i = 0; i < ulp_data.size(); ++i) {
    ...
    if (ulp_des > ulp_act)
      sub_ulp = ulp_des - ulp_act;
    if (!((fabs(act - des) <= small_num_threshold) ||
          (sub_ulp <= ulp_threshold) || (fabs((des - act) / des) <= 0.001))) {
      ++diff_elems_count;
      flag = false;
...
bool _cast_and_handle_fp_types(
    T1& data,
    T2& other,
    std::string name,
    [[maybe_unused]] double diff_elems_tol,
    size_t ulp_tol,
    double abs_tol) {
...
bool xetla_buff_cmp(
  ...
  if constexpr (
    ...
    if (std::is_same<remove_const_t<dtype1>, gpu::xetla::bf16>::value ||
        std::is_same<remove_const_t<dtype2>, gpu::xetla::bf16>::value) {
      return _cast_and_handle_fp_types<gpu::xetla::bf16>(
          data, other, name, diff_elems_tol, ulp_tol, abs_tol);
    } else if (
        std::is_same<remove_const_t<dtype1>, gpu::xetla::fp16>::value ||
        std::is_same<remove_const_t<dtype2>, gpu::xetla::fp16>::value) {
      return _cast_and_handle_fp_types<gpu::xetla::fp16>(
          data, other, name, diff_elems_tol, ulp_tol, abs_tol);
    } else if (
        std::is_same<remove_const_t<dtype1>, gpu::xetla::tf32>::value ||
        std::is_same<remove_const_t<dtype2>, gpu::xetla::tf32>::value) {
      return _cast_and_handle_fp_types<gpu::xetla::tf32>(
          data, other, name, diff_elems_tol, ulp_tol, abs_tol);
    } else if (
        std::is_same<remove_const_t<dtype1>, float>::value ||
