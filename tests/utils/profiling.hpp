...
class profiling_helper {
  ...
  void get_statistics(vector<double>& time, profiling_statistics& stat) {
    ...
    // median time
    int mid = int(iter / 2);
    stat.median = iter % 2 == 0 ? (time[mid] + time[mid + 1]) * 0.5 : time[mid];
  ...
  void print_statistics(
    ...
    for (uint32_t i = 0; i < value.size(); i++) {
      string info = "The " + desc[i] + "running(" + device + "_time) time is ";
  ...
  void print_performance(
    ...
    for (uint32_t i = 0; i < value.size(); i++) {
      string info =
          "The " + desc[i] + work_name[kernel_id] + "(" + device + "_time) is ";
      double perf = ((double)work_amount[kernel_id] / scaling_ratio) / value[i];
    ...
    ::testing::Test::RecordProperty(
        "kernel_time:" + work_name[kernel_id] + ":" +
  ...
  void set_time_vecs() {
    ...
    assert(
        (this->kernel_nums >= this->kernel_name.size()) &&
  ...
  void write_performance_metrics_into_report() {
    ...
    for (uint32_t kernel_id = 0; kernel_id < this->kernel_nums; kernel_id++) {
      for (uint32_t i = 0; i < metrics.size(); i++) {
        if (!is_printed[i] && (this->work_name[kernel_id] == metrics[i])) {
          ::testing::Test::RecordProperty(
              ":kernel_time:" + metrics[i], "minimum:maximum:median:mean");
          is_printed[i] = 1;
        }
      }
    }
 ...
 public:
  ...
  void cpu_end(int kernel_id = 0) {
    ...
    double cpu_time =
        (double)duration_cast<nanoseconds>(cpu_end_time - cpu_start_time)
            .count() /
        1000000;
