...
struct tile_load_store_local_func {
  static KERNEL_FUNC inline void run(
    ...
    payload_local_t slm_ld_payload((uint32_t)0, twidth, theight, twidth, 0, 0);
...
struct tile_load_store_vnni_local_func {
  static KERNEL_FUNC inline void run(
      ...
      dtype* c) {
    using tile_desc = tile_desc_t<twidth, theight, bwidth, bheight>;
    using vnni_tile_desc =
        tile_desc_t<twidth, theight, bwidth, bheight, reg_layout::vnni_tiled>;
    using mat_t = tile_t<dtype, tile_desc>;
...
struct tile_transpose_store_local_func {
  static KERNEL_FUNC inline void run(
    ...
    using tile_desc = tile_desc_t<
        ...
        bwidth,
        bheight,
        sizeof(dtype) == 4 ? reg_layout::tiled : reg_layout::vnni_tiled>;
    using mat_t = tile_t<dtype, tile_desc>;
    ...
    slm_store_payload_t local_store_A(
        (uint32_t)0, twidth, theight, twidth, 0, 0);
    slm_load_payload_t local_load_A((uint32_t)0, twidth, theight, twidth, 0, 0);
    mat_slm.reg = mat.reg;
