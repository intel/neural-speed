...
TEST(tile_load_store_vnni_local_func, esimd) {
  ...
  auto result_validate =
      std::bind(tile_load_store_result_validate<bf16>, _1, _2, _3, 128, 32, 32);
  kernel_run<
      bf16,
      tile_load_store_vnni_local_func<bf16, 128, 64, 128, 32, 32, 16, 16>>(
...
TEST(tile_load_store_local, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(tile_load_store_result_validate<int>, _1, _2, _3, 128, 32, 32);
  kernel_run<
      int,
...
TEST(tile_transpose_store_local, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(tile_load_store_result_validate<int>, _1, _2, _3, 64, 32, 32);
  kernel_run<
      int,
...
TEST(tile_transpose_store_local_bf16, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(tile_load_store_result_validate<bf16>, _1, _2, _3, 64, 32, 32);
  kernel_run<
      bf16,
...
TEST(tile_load_store_1row_local, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(tile_load_store_result_validate<int>, _1, _2, _3, 128, 32, 1);
  kernel_run<
      int,
...
TEST(tile_load_store_1d_local_with_update, esimd) {
  ...
  kernel_run<
      int,
      tile_load_store_1d_local_func<int, 128, 64, 128, 32, 32, 16, 16, 5>>(
      nd_range, result_validate);
}
