...
///------------------------------------------------------------------
/// Test global block load store default, using xetla_vector_ref instead of
/// xetla_vector Tested case:
/// - default block load store.
/// - xetla_load_global API with [dst] [2 src] [datatype int] [SIMD16].
/// - xetla_store_global API with [no return] [3 src] [datatype int] [SIMD16].
///------------------------------------------------------------------
...
/// Tested case:
/// - different datatype for xetla_load_global + xetla_store_global block
/// load/store.
/// - xetla_load_global API with [dst] [2 src] [different datatype] [SIMD16]
/// [data_size::default_size] [default L1 L2 cache hint].
/// - xetla_store_global API with [no return] [3 src] [different datatype]
/// [SIMD16] [data_size::default_size] [default L1 L2 cache hint].
///------------------------------------------------------------------
template <typename T>
...
INSTANTIATE_TYPED_TEST_SUITE_P(
    global_load_store_block,
    load_store_block_datatype_test,
    datatypes);
///------------------------------------------------------------------
...
/// - xetla_load_global API with [dst] [2 src] [datatype int] [SIMD16]
/// [data_size::default_size] [different L1 L2 cache hint].
///------------------------------------------------------------------
template <typename T>
...
///
using valid_load_cache_hints = ::testing::Types<
    std::tuple<None, None>,
    std::tuple<UC, UC>,
    std::tuple<UC, CA>,
    std::tuple<CA, UC>,
...
INSTANTIATE_TYPED_TEST_SUITE_P(
    global_load_store_block,
    load_block_cache_test,
    valid_load_cache_hints);
///------------------------------------------------------------------
...
///
using valid_store_cache_hints = ::testing::Types<
    std::tuple<None, None>,
    std::tuple<UC, UC>,
    std::tuple<UC, WB>,
    std::tuple<WT, UC>,
...
/// - xetla_prefetch_global API with [dst] [2 src] [datatype int] [SIMD16]
/// [data_size::default_size] [different L1 L2 cache hint].
///------------------------------------------------------------------
template <typename T>
...
/// Bspec link: https://gfxspecs.intel.com/Predator/Home/Index/53560
///
using valid_prefetch_cache_hints = ::testing::Types<
    std::tuple<UC, CA>,
    std::tuple<CA, UC>,
    std::tuple<CA, CA>,
    std::tuple<ST, UC>,
...
INSTANTIATE_TYPED_TEST_SUITE_P(
    global_load_store_block,
    prefetch_block_cache_test,
    valid_prefetch_cache_hints);
///------------------------------------------------------------------

///------------------------------------------------------------------
/// Test global prefetch datatype + datasize test
/// Tested case:
/// - different datatype for xetla_prefetch_global xetla_load_global +
/// xetla_store_global block load/store.
/// - xetla_prefetch_global API with [dst] [2 src] [different datatype] [SIMD16]
/// [data_size::default_size] [L1 L2 cache hint CA,CA].
/// - xetla_load_global API with [dst] [2 src] [different datatype] [SIMD16]
/// [data_size::default_size] [default L1 L2 cache hint].
/// - xetla_store_global API with [no return] [3 src] [different datatype]
/// [SIMD16] [data_size::default_size] [default L1 L2 cache hint].
