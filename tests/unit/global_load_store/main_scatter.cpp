...
/// scatter.
/// - xetla_load_global API with [dst] [2 src] [4 channel enabled] [different
/// datatype] [SIMD16] [data_size::default_size] [default L1 L2 cache hint].
/// - xetla_store_global API with [no return] [3 src] [all channel enabled]
/// [different datatype] [SIMD16] [data_size::default_size] [default L1 L2 cache
/// hint].
///------------------------------------------------------------------
template <typename T>
...
TYPED_TEST_P(load_scatter_datatype_test, esimd) {
  ...
  // For load mask kernel, the masked value would be 0 in buffer A and write to
  // bufferB
...
INSTANTIATE_TYPED_TEST_SUITE_P(
    global_load_store_scatter,
    load_scatter_datatype_test,
    datatypes);
...
/// - xetla_prefetch_global API with [dst] [2 src] [4 channel enabled]
/// [different datatype] [SIMD16] [data_size::default_size] [default L1 L2 cache
/// hint].
/// - xetla_load_global API with [dst] [2 src] [4 channel enabled] [different
/// datatype] [SIMD16] [data_size::default_size] [default L1 L2 cache hint].
...
INSTANTIATE_TYPED_TEST_SUITE_P(
    global_load_store_scatter,
    prefetch_scatter_datatype_test,
    datatypes);
///------------------------------------------------------------------
...
/// - different datatype for xetla_load_global + xetla_store_global block
/// scatter.
/// - xetla_load_global API with [dst] [2 src] [all channel enabled] [different
/// datatype] [SIMD16] [data_size::default_size] [default L1 L2 cache hint].
/// - xetla_store_global API with [no return] [3 src] [4 channel enabled]
/// [different datatype] [SIMD16] [data_size::default_size] [default L1 L2 cache
/// hint].
///------------------------------------------------------------------
template <typename T>
...
TYPED_TEST_P(store_scatter_datatype_test, esimd) {
  ...
  cl::sycl::nd_range<1> nd_range({1}, {1});
  // For store mask kernel, we write buffer B as value of SIMD in advance, so
  // masked channel value should be 16
  auto result_validate =
      std::bind(mask_result_validate<datatype>, _1, _2, _3, 16, 0xF, 16);
...
/// - xetla_store_global API with [no return] [3 src] [datatype int] [SIMD8]
/// [NElts2].
///------------------------------------------------------------------
