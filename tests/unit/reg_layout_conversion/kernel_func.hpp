...
struct conversion_func {
  static KERNEL_FUNC inline void run(
      ...
      dtype* c) {
    using linear_desc = subgroup::tile_desc_t<
        tile_size_x,
        tile_size_y,
        tile_size_x,
        tile_size_y,
        reg_layout::linear>;
    using tiled_desc = subgroup::tile_desc_t<
        tile_size_x,
        tile_size_y,
        block_size_x,
        block_size_y,
    ...
    payload_t result_payload(c, tile_size_x, tile_size_y, tile_size_x, 0, 0);
    ...
    auto linear_data_tile_2d =
        linear_data_tile.reg
            .xetla_format<native_type_t<dtype>, tile_size_y, tile_size_x>();
#pragma unroll
    for (uint32_t i = 0; i < tile_size_y; ++i) {
      linear_data_tile_2d.xetla_select<1, 1, tile_size_x, 1>(i, 0) += mask;
    }
