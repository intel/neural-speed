...
struct tile_load_store_func {
  static KERNEL_FUNC inline void run(
    ...
    mem_desc_t<dtype, mem_layout::row_major, mem_space::global> mem_desc_c(
        {c},
        {dst_swidth * ele_per_dw,
         dst_sheight / ele_per_dw,
...
struct tile_load_store_unaligned_2d_func {
  static KERNEL_FUNC inline void run(
    ...
    mem_desc_t<dtype, a_mem_layout, mem_space::global, 1> mem_desc_a(
        {a}, {dst_swidth, dst_sheight, src_spitch}, {0, 0});
    mem_desc_t<dtype, mem_layout::row_major, mem_space::global, 1> mem_desc_c(
        {c},
        {dst_swidth * ele_per_dw,
         dst_sheight / ele_per_dw,
         dst_spitch * ele_per_dw},
    ...
    // sycl::ext::oneapi::experimental::printf(vec_a, (matA.reg[0]),
    // (matA.reg[1]), (matA.reg[2]), (matA.reg[3]));
    // sycl::ext::oneapi::experimental::printf(vec_a, (matA.reg[16]),
    // (matA.reg[17]), (matA.reg[18]), (matA.reg[19]));
    matC.reg = matA.reg;
    tile_store(matC, payload_store);
    // static const char vec_b[] = "%d %d %d %d\n";
    // sycl::ext::oneapi::experimental::printf(vec_b, (matC.reg[0]),
    // (matC.reg[1]), (matC.reg[2]), (matC.reg[3]));
  }
};
...
struct tile_load_store_atomic_func {
  static KERNEL_FUNC inline void run(
    ...
    using check_tag_t = typename std::conditional<
        check_oob,
    ...
    matA_t matA;
    matBias_t matBias;
    payload_block_2d_t payload_load(a + offset, swidth, sheight, spitch, 0, 0);
    payload_block_2d_t payload_store(c + offset, swidth, sheight, spitch, 0, 0);
...
struct tile_load_broadcast_store_func {
  static KERNEL_FUNC inline void run(
      ...
      [[maybe_unused]] dtype* b,
      dtype* c) {
    using tile_desc_a = tile_desc_t<twidth, 1, bwidth, 1, reg_layout::tiled>;
    using tile_desc_c =
        tile_desc_t<twidth, theight, bwidth, bheight, reg_layout::tiled>;
...
struct tile_load_store_1d_func {
  static KERNEL_FUNC inline void run(
    ...
    if (check_boundary) {
      payload_load.template update_tdesc<tdesc_update_dir::y_dir>(33554432);
      payload_store.template update_tdesc<tdesc_update_dir::y_dir>(33554432);
    }
...
struct tile_load_store_oob_func {
  static KERNEL_FUNC inline void run(
    ...
    mem_desc_t<dtype, mem_layout::row_major, mem_space::global> mem_desc_c(
        {c}, {dst_swidth, dst_sheight, dst_spitch}, {offset_x, offset_y});
