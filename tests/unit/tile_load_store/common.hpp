...
int tile_load_store_result_validate(
    dtype* A,
  ...
  if (offset_y == 0) {
    ...
    for (unsigned i = 0; i < Blockx; ++i) {
      ...
      if (A[a_index] != C[a_index]) {
        if (++err_cnt < 100) {
          std::cout << "failed at index " << a_index << ", " << C[a_index]
                    << " != " << A[a_index] << "\n";
        }
      }
    }
...
int tile_load_broadcase_store_result_validate(
    dtype* A,
    [[maybe_unused]] dtype* B,
    dtype* C,
    unsigned Sizex,
...
template <typename dtype>
int tile_padding_load_store_result_validate(
    dtype* A,
    [[maybe_unused]] dtype* B,
    dtype* C,
    unsigned Sizex,
  ...
  for (unsigned i = 0; i < Sizey; ++i) {
    for (unsigned j = 0; j < Sizex; ++j) {
      ...
      if (a_temp != c_temp) {
        if (++err_cnt < 10) {
          std::cout << "failed at index [" << i << ", " << j << "]: " << c_temp
                    << " != " << a_temp << "\n";
        }
      }
    }
