...
TEST(bf16_block_load_store_cvt, esimd) {
  ...
  auto result_validate =
      std::bind(block_load_store_result_validate<bf16>, _1, _2, _3, 32, 16, 16);
...
TEST(tf32_block_load_store_cvt, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(block_load_store_result_validate<tf32>, _1, _2, _3, 32, 16, 16);
...
TEST(fp16_block_load_store_cvt, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(block_load_store_result_validate<fp16>, _1, _2, _3, 32, 16, 16);
