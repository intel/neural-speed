...
struct load_store_cvt_func {
  static KERNEL_FUNC inline void run(
    ...
    xetla_fill_tdesc<dtype, bwidth, bheight, 1>(
        a_src_tdesc.xetla_format<uint32_t>(), a, swidth, sheight, spitch, 0, 0);
    xetla_fill_tdesc<dtype, bwidth, bheight, 1>(
        b_src_tdesc.xetla_format<uint32_t>(), b, swidth, sheight, spitch, 0, 0);
    ...
    xetla_vector<dtype, bwidth* bheight> A_load_vec = xetla_tload_global<
        ...
        transform,
        arch_tag>(a_src_tdesc);
    xetla_vector<dtype, bwidth* bheight> B_load_vec = xetla_tload_global<
        dtype,
        bwidth * bheight,
        cache_hint::cached,
        cache_hint::cached,
    ...
    xetla_fill_tdesc<dtype, bwidth, store_height, 1>(
        dst_tdesc.xetla_format<uint32_t>(), c, swidth, sheight, spitch, 0, 0);
#pragma unroll
    for (int unroll_i = 0; unroll_i < bheight; unroll_i += store_height) {
      xetla_tstore_global<
          ...
          arch_tag>(
          dst_tdesc,
          out.xetla_select<bwidth * store_height, 1>(bwidth * unroll_i));
