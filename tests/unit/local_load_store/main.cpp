...
/// Tested case:
/// - different datatype for xetla_load_local + xetla_store_local block
/// load/store.
/// - xetla_load_local API with [dst] [1 src] [different datatype]  [NElts1]
/// [SIMD16] [data_size::default_size].
/// - xetla_store_local API with [no return] [2 src] [different datatype]
/// [NElts1] [SIMD16] [data_size::default_size].
///------------------------------------------------------------------
template <typename T>
...
TYPED_TEST_P(load_store_block_datatype_test, esimd) {
  ...
  auto result_validate =
      std::bind(local_load_store_result_validate<datatype>, _1, _2, _3, 16);
...
using datatypes = ::testing::Types<int, double, float>;
INSTANTIATE_TYPED_TEST_SUITE_P(
    local_load_store_scatter,
    load_store_scatter_datatype_test,
    datatypes);
///------------------------------------------------------------------
...
/// Tested case:
/// - different datatype for xetla_load_local + xetla_store_local scatter
/// load/store.
/// - xetla_load_local API with [dst] [1 src] [different datatype]  [NElts1]
/// [SIMD16] [4 channel enabled] [data_size::default_size].
/// - xetla_store_local API with [no return] [2 src] [different datatype]
/// [NElts1] [SIMD16] [all channel enabled] [data_size::default_size].
///------------------------------------------------------------------
...
TEST(local_load_scatter_mask, esimd) {
  ...
  kernel_run<int, local_load_scatter_mask<int, 16>>(nd_range, result_validate);
}
///------------------------------------------------------------------
...
/// load/store.
/// - xetla_load_local API with [dst] [1 src] [different datatype]  [NElts1]
/// [SIMD16] [all channel enabled] [data_size::default_size].
/// - xetla_store_local API with [no return] [2 src] [different datatype]
/// [NElts1] [SIMD16] [4 channel enabled] [data_size::default_size].
///------------------------------------------------------------------
...
TEST(local_store_scatter_mask, esimd) {
  ...
  kernel_run<int, local_store_scatter_mask<int, 16>>(nd_range, result_validate);
}
///------------------------------------------------------------------
...
/// - different datatype for xetla_load_local + xetla_store_local scatter
/// load/store.
/// - xetla_load_local API with [dst] [1 src] [different datatype]  [NElts2]
/// [SIMD16] [all channel enabled] [data_size::default_size].
/// - xetla_store_local API with [no return] [2 src] [different datatype]
/// [NElts2] [SIMD16] [4 channel enabled] [data_size::default_size].
///------------------------------------------------------------------
