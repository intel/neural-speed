...
struct block_load_store_func {
  static KERNEL_FUNC inline void run(
    ...
    xetla_fill_tdesc<dtype, bwidth, bheight, 1>(
        src_tdesc.xetla_format<uint32_t>(), a, swidth, sheight, spitch, 0, 0);
    ...
    constexpr int store_height = max_bytes_per_store / bwidth / sizeof(dtype);
...
#pragma unroll
    for (int unroll_i = 0; unroll_i < bheight; unroll_i += store_height) {
      xetla_tstore_global<
          ...
          arch_tag>(
          dst_tdesc,
          A_load_vec.xetla_select<bwidth * store_height, 1>(bwidth * unroll_i));
