...
int bit_shift_result_validate(
  ...
  for (uint32_t i = 0; i < Size; ++i) {
    auto lambda = [&, i]() -> std::tuple<datatype, datatype> {
      switch (op) {
        case bit_shift_op::shl_vector:
          return {A[i] << 1, C[i]};
        case bit_shift_op::shl_scalar:
          return {A[0] << 1, C[i]};
        case bit_shift_op::shr_vector:
          return {A[i] >> 1, C[i]};
        case bit_shift_op::shr_scalar:
          return {A[0] >> 1, C[i]};
        ...
        case bit_shift_op::ror_scalar:
          return {(A[0] >> 1) | (A[0] << 63), C[i]};
        case bit_shift_op::lsr_vector:
          return {A[i] >> 1, C[i]};
        case bit_shift_op::lsr_scalar:
          return {A[0] >> 1, C[i]};
        case bit_shift_op::asr_vector:
          return {A[i] >> 1, C[i]};
        case bit_shift_op::asr_scalar:
          return {A[0] >> 1, C[i]};
