...
TEST(tile_elemwise_op_relu, esimd) {
  ...
  auto result_validate = std::bind(
      tile_elemwise_op_validate<float, relu_op<float>>,
      _1,
      _2,
      _3,
      128,
  ...
  kernel_run<
      float,
      tile_elemwise_op_func<float, 128, 64, 128, 16, 16, 16, 16, relu_op_t>>(
...
TEST(tile_elemwise_op_gelu_fwd, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate = std::bind(
      tile_elemwise_op_validate<float, gelu_op<float>>,
      _1,
      _2,
      _3,
      128,
...
TEST(tile_elemwise_op_gelu_fwd_w, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate = std::bind(
      tile_elemwise_op_validate<float, gelu_fwd_w_op<float>>,
      _1,
      _2,
      _3,
      128,
...
TEST(tile_elemwise_op_res_add, esimd) {
  ...
  auto result_validate =
      std::bind(tile_elemwise_res_add_validate<float>, _1, _2, _3, 128, 16, 24);
  kernel_run<
      float,
      tile_elemwise_op_func<
          float,
