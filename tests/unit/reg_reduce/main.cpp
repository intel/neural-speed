...
TEST(test_reduce_add_fp32, esimd) {
  ...
  auto result_validate =
      std::bind(kernel_validation<float, reduce_sum<float>>, _1, _2, _3, 64);
...
TEST(test_reduce_mul_fp32, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(kernel_validation<float, reduce_prod<float>>, _1, _2, _3, 64);
...
TEST(test_reduce_min_fp32, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(kernel_validation<float, reduce_min<float>>, _1, _2, _3, 64);
...
TEST(test_reduce_max_fp32, esimd) {
  cl::sycl::nd_range<1> nd_range({1}, {1});
  auto result_validate =
      std::bind(kernel_validation<float, reduce_max<float>>, _1, _2, _3, 16);
