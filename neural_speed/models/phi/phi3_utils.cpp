...
void model_load_internal(const std::string& fname, model_archs arch, model_context* ctx, int n_gpu_layers,
  ...
  std::unique_ptr<phi3> ms(new phi3());
...
void phi3::init(const char* path_model, model_context* ctx, int n_gpu_layer_, bool use_mmap_, bool use_mlock_,
  ...
  scratch = phi3_mem_req(n_layer, lctx.scratch_size_ratio);
  model.scratchs = scratch;
}

#define MODEL_BACKEND_OFFLOAD NE_BACKEND_CPU
void phi3::load(model_context* ctx, model_progress_callback progress_callback, void* progress_callback_user_data) {
  model_context& lctx = *ctx;
  auto& model = lctx.model;
  ...
  size_t vram_total = 0;
  // PHI3 is set up so that if padding_idx is specified then offset the embedding ids by 2
...
class phi3_quant_layer : public quant_layer_base {
 public:
...
REGISTER_QUANT_LAYER_CLASS(phi3);
