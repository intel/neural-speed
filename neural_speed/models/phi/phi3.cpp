...
static bool phi3_model_eval_internal(model_context* ctx, const model_input* inputs, const int n_input,
                                     const int n_threads) {
  ...
  const int64_t batch_size = lctx.batch_size;
  ...
  const int64_t n_ctx = lctx.n_ctx;
...
int model_eval(struct model_context* ctx, const model_input* inputs, const int n_input, int n_threads) {
  if (!phi3_model_eval_internal(ctx, inputs, n_input, n_threads)) {
