...
#include "models/chatglm/chatglm3.h"
...
void model_load_internal(const std::string& fname, model_archs arch, model_context* ctx, int n_gpu_layers,
  ...
  std::unique_ptr<CHATGLM3> ms(new CHATGLM3());
...
void CHATGLM3::init(const char* path_model, model_context* ctx, int n_gpu_layer_, bool use_mmap_, bool use_mlock_,
                    bool vocab_only_) {
  model_context& lctx = *ctx;
...
void CHATGLM3::load(model_context* ctx, model_progress_callback progress_callback, void* progress_callback_user_data) {
  model_context& lctx = *ctx;
  auto& model = lctx.model;
...
class chatglm3_quant_layer : public quant_layer_base {
 public:
...
REGISTER_QUANT_LAYER_CLASS(chatglm3);
