...
def main(args_in: Optional[List[str]] = None) -> None:
    ...
    for i in range(hparams["n_layer"]):
        ...
        #                             f"{prefix}.self_attn.k_proj.weight", list_vars, fout, quantize_config)
        # convert_to_qx_bestla_tensor(f"{prefix}.self_attn.v_proj.weight",
        #                             f"{prefix}.self_attn.v_proj.weight", list_vars, fout, quantize_config)
        # convert_to_qx_bestla_tensor(f"{prefix}.self_attn.o_proj.weight",
        #                             f"{prefix}.self_attn.o_proj.weight", list_vars, fout, quantize_config)
        # convert_to_qx_bestla_tensor(f"{prefix}.self_attn.dense.weight",
        #                             f"{prefix}.self_attn.dense.weight", list_vars, fout, quantize_config)

        # convert_qwen_to_fp32_tensor(f"{prefix}.self_attn.q_proj.bias",
        #                             f"{prefix}.self_attn.q_proj.bias", list_vars, fout)
        # convert_qwen_to_fp32_tensor(f"{prefix}.self_attn.k_proj.bias",
        #                             f"{prefix}.self_attn.k_proj.bias", list_vars, fout)
        # convert_qwen_to_fp32_tensor(f"{prefix}.self_attn.v_proj.bias",
        #                             f"{prefix}.self_attn.v_proj.bias", list_vars, fout)
        # convert_qwen_to_fp32_tensor(f"{prefix}.self_attn.dense.bias",
