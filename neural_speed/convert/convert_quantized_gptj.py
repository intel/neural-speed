...
def convert_q4_bestla_tensor(src_name, dst_name, model, fout, q_config):
    ...
    dst = np.zeros(
        (int_weight.shape[0], int_weight.shape[1] * 4), dtype=np.int8)
...
def main(args_in: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        "--outtype", choices=["f32", "f16"], help="output format (default: based on input)")
    ...
    fout.write(struct.pack(
        "i", tokenizer.bos_token_id if tokenizer.bos_token_id is not None else 1))
    fout.write(struct.pack(
        "i", tokenizer.eos_token_id if tokenizer.eos_token_id is not None else 2))
    fout.write(struct.pack(
        "i", tokenizer.pad_token_id if tokenizer.pad_token_id is not None else -1))
    fout.write(struct.pack(
        "i", tokenizer.sep_token_id if tokenizer.sep_token_id is not None else -1))
    ...
    convert_fp32_tensor("transformer.wte.weight",
                        "transformer.wte.weight", list_vars, fout)
    convert_fp32_tensor("transformer.ln_f.weight",
                        "transformer.ln_f.weight", list_vars, fout)
