...
def main(args_in: Optional[List[str]] = None) -> None:
    ...
    for i in tqdm(range(n_layer), desc="Processing layers"):
        convert_to_qx_bestla_tensor(f"transformer.h.{i}.attn.q_proj.weight", f"transformer.h.{i}.attn.q_proj.weight",
                                    list_vars, fout, quantize_config)
        convert_to_qx_bestla_tensor(f"transformer.h.{i}.attn.k_proj.weight", f"transformer.h.{i}.attn.k_proj.weight",
                                    list_vars, fout, quantize_config)
        convert_to_qx_bestla_tensor(f"transformer.h.{i}.attn.v_proj.weight", f"transformer.h.{i}.attn.v_proj.weight",
                                    list_vars, fout, quantize_config)
        ...
        convert_to_qx_bestla_tensor(f"transformer.h.{i}.mlp.fc_in.weight", f"transformer.h.{i}.mlp.fc_in.weight",
                                    list_vars, fout, quantize_config)
        convert_to_qx_bestla_tensor(f"transformer.h.{i}.mlp.fc_out.weight", f"transformer.h.{i}.mlp.fc_out.weight",
                                    list_vars, fout, quantize_config)

        convert_to_fp32_tensor(f"transformer.h.{i}.mlp.fc_in.bias", f"transformer.h.{i}.mlp.fc_in.bias", list_vars,
                               fout)
        convert_to_fp32_tensor(f"transformer.h.{i}.mlp.fc_out.bias", f"transformer.h.{i}.mlp.fc_out.bias", list_vars,
                               fout)

        convert_to_fp32_tensor(f"transformer.h.{i}.ln_1.weight", f"transformer.h.{i}.ln_1.weight", list_vars, fout)
        convert_to_fp32_tensor(f"transformer.h.{i}.ln_1.bias", f"transformer.h.{i}.ln_1.bias", list_vars, fout)
